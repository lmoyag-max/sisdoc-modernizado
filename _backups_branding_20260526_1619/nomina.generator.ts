...
export function generarNominaPDF(data: NominaData): jsPDF {
  ...
  const azulOsc:[number, number, number] = [29,  78,  216];
  const grisF:  [number, number, number] = [243, 244, 246];
  const grisL:  [number, number, number] = [107, 114, 128];
  ...
  doc.text('HOSPITAL UNIVERSITARIO ASOCIADO DE PUEBLA', mg + cW / 2, y + 8, { align: 'center' });
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text('Sistema de Gestión Documental — SISDOC', mg + cW / 2, y + 14.5, { align: 'center' });
  y += 24;

  // ── Título nómina ──────────────────────────────────────────
  doc.setFillColor(...grisF);
  doc.rect(mg, y, cW, 13, 'F');
  doc.setDrawColor(...azulOsc);
  ...
  doc.text(`Generado el ${ahora} — SISDOC HUAP`, mg + cW / 2, y, { align: 'center' });
