...
import { Send, Clock, ChevronLeft, ChevronRight, FileText, CheckCircle2 } from 'lucide-react';
...
const ESTADO_BADGE: Record<number, { label: string; variant: 'info' | 'warning' | 'success' | 'secondary' }> = {
  1: { label: 'Pendiente recepción', variant: 'warning' },
  2: { label: 'Recepcionado',        variant: 'info' },
...
export function EnviadosPage() {
  ...
  const completados = tramites.filter((t) => t.id_estado_tramite === 3).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Pendientes',     value: tramites.filter((t) => t.id_estado_tramite === 1).length, color: 'text-amber-600' },
          { label: 'Recepcionados',  value: tramites.filter((t) => t.id_estado_tramite === 2).length, color: 'text-blue-600' },
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <EmptyState
              icon={Send}
              title="Sin envíos"
              description="No has enviado documentos aún."
            />
          ) : (
            <div className="divide-y">
              {tramites.map((t) => {
                const badge = ESTADO_BADGE[t.id_estado_tramite ?? 0];
                const isCompleto = t.id_estado_tramite === 3;
