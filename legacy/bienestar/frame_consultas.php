...
$query="SELECT * FROM comunas where reg_codigo = " . $id_dep . " order by com_descripcion";
...
echo "<script>\n";
$total_rec = $Totreg+1;
echo " parent.mainFrame.document.form1.cbo_comuna.options.length=" . $total_rec . ";\n";
echo " parent.mainFrame.document.form1.cbo_comuna.options[0].value='0';\n";
echo " parent.mainFrame.document.form1.cbo_comuna.options[0].text='-------';\n";

while($reg_comuna = mssql_fetch_array($rs_comuna))
   {
       $i = $i + 1;
       echo " parent.mainFrame.document.form1.cbo_comuna.options[" . $i . "].value='" . $reg_comuna[com_codigo] . "';\n";
       echo " parent.mainFrame.document.form1.cbo_comuna.options[" . $i . "].text='" . $reg_comuna[com_descripcion] .  "';\n";
...
// Busca si existe el rut de la carga ingresada
if ($sw=="er")
{
if(isset($id_dep)) {
$aux_rut=$cod_dep;
$rut_carga= substr($aux_rut,0,-1);
...
$cc = mssql_connect("bd2-minsal", "bienes", "bienes2004") or die("El Servidor No se encuentra");
	...
	//echo "rut_carga " . $rut_carga;
	
$query_carga="SELECT * FROM carga_fam where rut_carga = " . $rut_carga ;
$rs_carga = mssql_query($query_carga, $cc);
//$descriptor_query="exec ingreso_descriptor '" . $vector[$x] . "','" . $Id_Documento . "','" . $op . "'";
//$rs_funcionario2 = mssql_query("exec busca_funcionario " . $id_dep, $cn);

$Totreg = mssql_num_rows($rs_carga);

if($Totreg>0)
{
echo "<script>\n";
echo " alert('El Rut que esta ingresando ya existe');\n";
echo " parent.mainFrame.document.form1.el_rut.select() ;\n";
echo " parent.mainFrame.document.form1.el_rut.focus() ;\n";
echo "</script>\n";	
$reg_carga.close;
$rs_carga.close;
mssql_close($cc);
...
if ($sw=="BP")
 {
//echo "ext" .  $desd . ",proc" .  $prod . ",tipo doc" . $id_dep ;
$rs_doc="exec busca_externo '" . $desd . "','" . $prod . "','" . $id_dep . "'";

$rs_documento=mssql_query($rs_doc,$cn);  
//$reg_doc=mssql_fetch_array($rs_documento); 
$Totreg = mssql_num_rows($rs_documento);
//echo "ext" .  $desd . ",proc" .  $prod . ",tipo doc" . $id_dep . "tot " . $Totreg;
if($Totreg==0)
{
    echo "<html>\n";
...
// ***************** Busca Destino para oficina de partes ******************************
if ($sw=="TD" and $nom_dep !="")
 {
$rs_destino = mssql_query("SELECT * FROM dependencia order by desc_dependencia", $cn);
$rs_dep1= mssql_query("select id_dependencia from dependencia where cod_dependencia = '" . $nom_dep . "'", $cn);
$Totreg = mssql_num_rows($rs_dep1);
$reg_dep1 = mssql_fetch_array($rs_dep1);
...
if(isset($id_dep)) {
$op="I";
$query="SELECT * FROM funcionario where id_dependencia = " . $id_dep;
