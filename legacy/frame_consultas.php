...
// busca tema  asociado a la procedencia que se ha ingresado como codigo y no como combo
else 
{
$rs_tema = mssql_query("select a.id_tema,a.desc_tema from temas_alertas a ,tipo_compromiso2 b ,dependencia_externa c where c.id_dependencia_externa=b.id_dependencia  and b.id_tema=a.id_tema  and c.cod_dependencia_externa= '" . $nom_dep . "'". " order by a.desc_tema", $cn);
$Totreg = mssql_num_rows($rs_tema);
$total_rec=$Totreg+1;
echo " parent.mainFrame.document.form1.Cbo_tema.options.length= " . $total_rec . ";\n";
echo " parent.mainFrame.document.form1.Cbo_tema.options[0].value='0';\n";
echo " parent.mainFrame.document.form1.Cbo_tema.options[0].text='-----Seleccione tema --------';\n";
$i=0;
while($reg_tema = mssql_fetch_array($rs_tema))
   {
        $i = $i + 1;
       echo "parent.mainFrame.document.form1.Cbo_tema.options[" . $i . "].value='" . $reg_tema[id_tema] . "';\n";
       echo "parent.mainFrame.document.form1.Cbo_tema.options[" . $i . "].text='" . $reg_tema[desc_tema] . "';\n";
    }
}
echo "</script>\n";
$reg_dep1.close;
...
//--------Busca teniendo el codigo asociarlo en el combo y mostrar el tema respectivo  como codigo desde oficina de partes -------//
 if ($sw=="BT" and $nom_dep !="")
 {  
$rs_tem = mssql_query("select a.id_tema,a.desc_tema,b.plazo_limite_dias from temas_alertas a ,tipo_compromiso2 b ,dependencia_externa c  where c.cod_dependencia_externa=$dep and c.id_dependencia_externa=b.id_dependencia   and b.id_tema = a.id_tema order by a.desc_tema", $cn);
$rs_tem1= mssql_query("select a.id_tema from temas_alertas a where a.id_tema  = '" . $nom_dep . "'", $cn);
$Totreg = mssql_num_rows($rs_tem1);
$Tottem= mssql_num_rows($rs_tem);
if ($Tottem <> 0)
{
	$reg_tem1 = mssql_fetch_array($rs_tem1);
	$idtem=$reg_tem1["id_tema"];
	 //echo "tema " . $idtem . "dep2" . $dep . "nom_dep " . $nom_dep ; 
	echo "<script>\n";
	$i=1;
	while($reg_tema = mssql_fetch_array($rs_tem))
	{
	  if($idtem==$reg_tema[id_tema]) 
	   	echo " parent.mainFrame.document.form1.Cbo_tema.selectedIndex='" . $i ."';\n";
		echo " parent.mainFrame.document.form1.Cbo_tema.options[" . $i . "].selected;\n";	 
  		$i = $i + 1;
	}

	if($idtem =="")
	{
	echo "alert('Código no está asociado a procedencia');";

	echo " parent.mainFrame.document.form1.Cbo_tema.options[0].value='0';\n";
    echo " parent.mainFrame.document.form1.Cbo_tema.options[0].text='-----Seleccione tema --------';\n";
    $tema =0;
	echo " parent.mainFrame.document.form1.Txttema.value='" . $tema ."';\n"; 
    }
	if ($idtem==0  )
     {	
	    echo "alert('Debe ingresar código ');"; 
	    $i= 2;
		echo " parent.mainFrame.document.form1.Cbo_tema.selectedIndex='" . $i ."';\n";
		echo " parent.mainFrame.document.form1.Cbo_tema.options[" . $i . "].selected;\n";
		echo " parent.mainFrame.document.form1.Txttema.value=parent.mainFrame.document.form1.Cbo_tema.options[" . $i . "].value";
...
{
		echo "<script>\n";
		echo "alert('No tiene temas asociados');";
		echo " parent.mainFrame.document.form1.Cbo_tema.options[0].value='0';\n";
		echo " parent.mainFrame.document.form1.Cbo_tema.options[0].text='-----Seleccione tema --------';\n";
	    $tema =0;	
		echo " parent.mainFrame.document.form1.Txttema.value='" . $tema ."';\n";
...
// ***************** Busca procedencia en tabla de tipo_compromiso2 para obtener tema asociado a procedencia desde ingreso of. partes ******************************

if ($sw=="OT" and $cod !="")
 { 
 
$rs_tema = mssql_query("select a.id_tema,a.desc_tema from temas_alertas a ,tipo_compromiso2 b ,dependencia_externa c where c.id_dependencia_externa=b.id_dependencia  and b.id_tema=a.id_tema  and c.cod_dependencia_externa= '" . $cod . "'". " order by a.desc_tema", $cn);
$Totreg = mssql_num_rows($rs_tema);
echo "<script>\n";
$total_rec=$Totreg+1;
echo " parent.mainFrame.document.form1.Cbo_tema.options.length= " . $total_rec . ";\n";
...
$reg_tema.close; 
 ...
 //--------Busca teniendo el codigo asociarlo en el combo y mostrar el tema respectivo  como codigo desde oficina de partes -------//
if ($sw=="OTE" and $nom_dep !="")
 {  
$rs_plazo= mssql_query("select a.id_tema,a.desc_tema,b.plazo_limite_dias,b.id_compromiso from temas_alertas a ,tipo_compromiso2 b ,dependencia_externa c  where c.cod_dependencia_externa=$dep and c.id_dependencia_externa=b.id_dependencia   and b.id_tema = a.id_tema  and b.id_tema = $cod  order by a.desc_tema", $cn);
$Totreg = mssql_num_rows($rs_plazo);
$reg_plazo = mssql_fetch_array($rs_plazo);
$plazolim=$reg_plazo["plazo_limite_dias"];
$idcomp =$reg_plazo["id_compromiso"];
// buscando si la procedencia tiene temas asociados 
$rs_tema = mssql_query("select a.id_tema,a.desc_tema from temas_alertas a ,tipo_compromiso2 b ,dependencia_externa c where c.id_dependencia_externa=b.id_dependencia  and b.id_tema=a.id_tema  and c.cod_dependencia_externa= '" . $dep . "'". " order by a.desc_tema", $cn);
$Tottema= mssql_num_rows($rs_tema);
$i=0;
if (($Tottema<>0)  && ( $cod==0))
  { 
  echo "<script>\n";
  echo "alert('Debe elegir un tema ');";
  echo ' parent.mainFrame.document.form1.Txttema.focus();' . ";\n";
  echo "</script>\n";
  
}
echo "<script>\n";
echo " parent.mainFrame.document.form1.TxtDias.value='" . $plazolim ."';\n";
echo " parent.mainFrame.document.form1.idcomp.value='" . $idcomp ."';\n";
echo "</script>\n";
$reg_plazo.close;
$rs_plazo.close;
}
else
// busca que los dias de compromiso ingresados no sea mayor a los dias que tiene como limite  trae el dia decompromiso que se ha seleccionado en ese momento
// $dd es el dia que se ingresa por la pantalla , $idcomp es el compromiso que se eligio a traves de  procedencia y de el tema 
if ($sw=="VD")
{
if ($idcomp <>0 )
{
	$rs_dias = mssql_query("select plazo_limite_dias from tipo_compromiso2  where id_compromiso = '" . $idcomp . "'", $cn);
	$rs_dd    =mssql_fetch_array($rs_dias);
	$dialimite= $rs_dd["plazo_limite_dias"];
	if ($dialimite < $dd )
	{echo "<script>\n";
	echo "alert('Dias ingresado es mayor al límite para ese tema y esa dependencia  ');";
	echo " parent.mainFrame.document.form1.TxtDias.value='" . $dialimite ."';\n";
	echo ' parent.mainFrame.document.form1.TxtDias.focus();' . ";\n";
	echo "</script>\n";
	 } 
 }
}
...
//se cambia para que considere ademas solamente los que están vigentes de los que tienen acceso desde oficina de partes 04/02/2009
//$query = "select id_dependencia, SUBSTRING(desc_dependencia, 1, 35) AS desc_dependencia, cod_dependencia_nuevo from dependencia where  vigencia is NULL and ofpartes='S'   order by desc_dependencia  ";
...
// se cambia para que tome ademas solo los  vigentes 04/02/2009
$query = "select id_dependencia from dependencia where cod_dependencia_nuevo = '" . $nom_dep . "'" . " and ofpartes=" . "'S'"  ." and vigencia is NULL ";
...
// busca las dependencias internas para el combo en la pantalla de reenviar para oficina de partes
if ($sw=="DF" && $nom_dep=="I")
{
    //echo "entra" . $entra.  "iddep" . $id_dep . "<br>"; 
	if(isset($id_dep))
	{
	// busca  el id_dependencia de la dependencia escogida en el combo //
	$ofp ="S";
	$rs_dep=mssql_query("SELECT id_dependencia FROM dependencia where vigencia is NULL   and ofpartes = ". "'S'" . " and  cod_dependencia_nuevo = " . "'$id_dep'", $cn);
	$reg_dep =mssql_fetch_array($rs_dep);
	$dep=$reg_dep["id_dependencia"];
	//echo  "dep" . $dep . "iddep" . $iddep ;
	// para  que muestre solo funcionarios vigentes // 
	$rs_funcionario=mssql_query("SELECT * FROM funcionario where vigencia is NULL  and id_dependencia = " . $dep .  "order by nombres,apellidos", $cn);
	$Totreg = mssql_num_rows($rs_funcionario);
	$i=0;
	echo "<script>\n";
...
// busca las dependencias externas para el combo en la pantalla de reenviar para oficina de partes
if ($sw=="DF"  && $nom_dep=="E")
{
$nom_dep=57;
$rs_dep = mssql_query("select id_dependencia_externa, SUBSTRING(desc_dependencia_externa, 1, 35) AS desc_dependencia_externa, cod_dependencia_externa
...
// Es parea efectos de ingresar el codigo en reenviar como oficina de partes y se asocie a su respectivo combo destino 
// echo "nom_dep  " . $nom_dep . "id_dep  " . $id_dep . "sw" . $sw;
 if ($sw=="OP" && $nom_dep !="")
 {
// se  cambia para que tome solo los que se requieren para oficina de partes y no todo el listado  y solo los que estan  vigentes 
if ($id_dep=="I")
...
if ($id_dep=="E")
   ...
   {
	$query = "select id_dependencia_externa, SUBSTRING(desc_dependencia_externa, 1, 35) AS desc_dependencia_externa from dependencia_externa  order by desc_dependencia_externa";
	$rs_destino =mssql_query($query, $cn);
	$query = "select id_dependencia_externa from dependencia_externa where cod_dependencia_externa = '" . $nom_dep . "'"  ;
	$rs_dep1= mssql_query($query, $cn);
	$Totreg = mssql_num_rows($rs_dep1);
	$reg_dep1 = mssql_fetch_array($rs_dep1);
	$iddes=$reg_dep1["id_dependencia_externa"];
	}
echo "<script>\n";
$i=0;
while($reg_destino = mssql_fetch_array($rs_destino))
 {
//    echo $iddes . "re   " . $reg_destino[id_dependencia] . "<br>";
  if ($iddes==$reg_destino[id_dependencia]) 
    {
	echo "parent.mainframe.document.form1.Cbo_Destinatario.selectedIndex='" . $i . "';\n";
    echo "parent.mainframe.document.form1.Cbo_Destinatario.options[" . $i ."·].selected;\n",
...
/*if($iddes =="")
{
echo "alert('Código no existe ttt');";
echo ' parent.mainFrame.document.form1.Txtdestino.value="";' . ";\n";
}*/
...
// permite recatar nombre de la dependencia para imprimir en la nomina que se genera  viene en  dato el id_emisor 
if ($sw=="IN")
{
$rs_query = "select desc_dependencia from  dependencia  where id_dependencia = '" . $dato. "'" ;
$reg= mssql_query($rs_query,$cn);
$reemisor =mssql_fetch_array($reg);
echo $reemisor;
echo " parent.mainFrame.document.form1.nombreemisor.value='" . $reemisor ."';\n";
}
// permite obtener  los datos del documento para relacionar 
if ($sw=="BD" )
{ 
   $rs_documento="exec documento_referencia '" . $docu . "','" . $segu . "'";
   $qq = mssql_query($rs_documento,$cn); 
...
//   $xprocedencia=$rs[procedencia];
  ...
  // buscando datos de la procedencia inicial 
   $rs_proc="exec busca_tramite_minimo '" . $docu . "'";
   $qp = mssql_query($rs_proc,$cn); 
   $rp=mssql_fetch_array($qp);
   $xprocedencia=$rp[procedencia];
   
 if ($num_doc ==1 )
 {
	echo "<script>\n"; 
  echo " parent.mainFrame.document.form1.doc1.value='" . $docu."';\n";
  echo " parent.mainFrame.document.form1.desc_tipo_documento1.value='" . $xtipodoc ."';\n";
  echo " parent.mainFrame.document.form1.num_interno1.value='" . $xnuminterno."';\n";
  echo " parent.mainFrame.document.form1.num_oficial1.value='" . $xnumoficial ."';\n";
  echo " parent.mainFrame.document.form1.num_externo1.value='" . $xnumexterno1 ."';\n";
  echo " parent.mainFrame.document.form1.original1.value='" . $xoriginal ."';\n";
  echo " parent.mainFrame.document.form1.fecha_documento1.value='" . $xfech_doc ."';\n";
  echo " parent.mainFrame.document.form1.procedencia1.value='" . $xprocedencia ."';\n";
  echo " parent.mainFrame.document.form1.materia1.value='" . $xmateria ."';\n";  
  echo " parent.mainFrame.document.form1.medio1.value='" . $xmedio ."';\n";  
 ...
 {
  echo "<script>\n"; 
  echo " parent.mainFrame.document.form1.doc2.value='" . $docu."';\n";
  echo " parent.mainFrame.document.form1.desc_tipo_documento2.value='" . $xtipodoc ."';\n";
  echo " parent.mainFrame.document.form1.num_interno2.value='" . $xnuminterno."';\n";
  echo " parent.mainFrame.document.form1.num_oficial2.value='" . $xnumoficial ."';\n";
  echo " parent.mainFrame.document.form1.num_externo2.value='" . $xnumexterno1 ."';\n";
  echo " parent.mainFrame.document.form1.original2.value='" . $xoriginal ."';\n";
  echo " parent.mainFrame.document.form1.fecha_documento2.value='" . $xfech_doc ."';\n";
  echo " parent.mainFrame.document.form1.procedencia2.value='" . $xprocedencia ."';\n";
  echo " parent.mainFrame.document.form1.materia2.value='" . $xmateria ."';\n";  
  echo " parent.mainFrame.document.form1.medio2.value='" . $xmedio ."';\n";  
  echo "</script>\n"; 
 }
}
//permite verificar si el documento tiene  validar que el primer documento tenga destino hacia alguna dependencia del usuario , 
//que el segundo documento sea de alguna dependencia del usuario
// y que  no exista relación previa de los documentos , en caso que todo esto se cumpla va a permitir relacionar los documentos 
 if ($sw=="BR")
 {
     // validando  que el documento 1 tenga  algun destino o procedencia  que tiene acceso el usuario  
	     $d=0;
		
		 $existe_usuario ="select * from tramite a , acceso   b  where  b.id_usuario=". $idusuario . " and ((b.id_dependencia = a.id_destino  and a.tipo_destinatario='I') or (b.id_dependencia =a.id_procedencia and a.tipo_procedencia='I')) and a.id_documento=". $doc1 ;
//		echo "query" . $existe_usuario . "<br>";
		 $rusuario=mssql_query($existe_usuario,$cn);
		 $tot=mssql_num_rows($rusuario);
		if ($tot >1 || $tot == 1)
		 { $d=1;}   
              // validando  que el documento 1 tenga  algun destino o procedencia  que tiene acceso el usuario  
	     $p=0;
		 $existe_procedenc ="select * from tramite a , acceso   b  where b.id_usuario=". $idusuario . "  and ((b.id_dependencia = a.id_destino  and a.tipo_destinatario='I') or (b.id_dependencia =a.id_procedencia and a.tipo_procedencia='I'))  and a.id_seguimiento in (select min(id_seguimiento) from tramite where id_documento=$doc2)
                             and a.id_documento=". $doc2;
						//	  echo $existe_procedenc;
		 $rproc=mssql_query($existe_procedenc,$cn);
		 $registrop=mssql_fetch_array($rproc);
		 $procedencia = $registrop[id_procedencia];
		 
		 $totp=mssql_num_rows($rproc);
		if ($totp >1 || $totp == 1)
		 { $p=1;}   

      // Cambiando estado del tramite del 1er documento cuando el 2do. documento 
	  //tiene  como primera procedencia alguna dependencia del usuario y que este a su vez este como destino en el 1er documento.
	    // cambia su estado de tramite si está recpcionado para que quede como derivado 
		$buscaproc ="update tramite  set id_estado_tramite=4 where  id_destino= ". $procedencia . " and tipo_destinatario='I' and id_estado_tramite=3   and id_documento=". $doc1 ;
		$rproc=mssql_query($buscaproc);
	 // buscar si ya existe relacion , en caso contrario ingresar relacion de documentos 
	  if ($d == 1 && $p==1)
	   { $ps_busca_asociados = "select * from llama_historia($doc1) order by id_documento,id_seguimiento ";
	     $rs_asocia = mssql_query($ps_busca_asociados,$cn); 
		 $sw_e=0;
	     while($re=mssql_fetch_array($rs_asocia) )
		   {
	       if ($sw_e==1)
			     {break;}
		   if ($re[id_documento] == $doc2)
		      {
				echo "<script>\n";
				echo "alert('Ya están relacionados');";
				echo "</script>\n";
				$sw_e=1;
			  }
		 } 
    	 if ($sw_e==0) // no hay relación de documentos , se deben asociar en tabla relacion_documento
		 { 
			$graba_relacion ="insert into relacion_documento(id_documento,id_documento_hijo) values(".$doc1 .','. $doc2 . ')';
			$rg =mssql_query($graba_relacion,$cn);
		   	echo "<script>\n";
			echo "alert('Documento 1 y documento 2 quedaron asociados en el sistema ');";
			echo "</script>\n";
		 }	
   }
   else  if ($d==0)
       	{
		 echo "<script>\n";
		 echo "alert('Usted no tiene acceso a relacionar estos documentos, el documento 1 no fue enviado a una(s) de sus dependencia(s)');";
		 echo "</script>\n";
	     }
   else  if ($p==0)
       	{
		 echo "<script>\n";
		 echo "alert('El documento 2 no es de su(s) dependencias(s)');";
		 echo "</script>\n";
	     }
 }  
?>
