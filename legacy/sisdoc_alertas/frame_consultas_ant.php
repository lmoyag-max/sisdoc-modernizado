...
//		echo "query" . $existe_usuario;
		...
		if ($tot >1 || $tot == 1)
		 { $d=1;}   
              // validando  que el documento 1 tenga  algun destino o procedencia  que tiene acceso el usuario  
	     $p=0;
		 $existe_procedenc ="select * from tramiA?????ŸGo???????te a , acceso   b  where a.id_usuario=b.id_usuario  and (b.id_dependencia = a.id_destino or b.id_dependencia =a.id_procedencia)  and a.id_seguimiento in (select min(id_seguimiento) from tramite where id_documento=$doc2)
