...
if ($sw=="BD" )
{ 
   echo "documento" . $xtipodoc . "sdsdsds". $xnuminterno;
   $rs_documento="exec documento_referencia '" . $docu . "','" . $segu . "'";
   $qq = mssql_query($rs_documento,$cn); 
   $rs=mssql_fetch_array($qq);
   $xtipodoc =$rs[desc_tipo_documento];
...
// y que  no exista relación previa de los documentos , en caso que todo esto se cumpla va a permitir relacionar los documentos 
 if ($sw=="BR")
 {
     // validando  que el documento 1 tenga  algun destino o procedencia  que tiene acceso el usuario  
		 ...
		 $existe_usuario ="select * from tramite a , acceso   b  where  a.id_usuario=b.id_usuario  and (b.id_dependencia = a.id_destino or b.id_dependencia =a.id_procedencia) and a.id_documento=". $doc1;
//		echo "query" . $existe_usuario;
		...
		if ($tot >1 || $tot == 1)
		 { $d=1;}   
              // validando  que el documento 1 tenga  algun destino o procedencia  que tiene acceso el usuario  
	     $p=0;
		 $existe_procedenc ="select * from tramite a , acceso   b  where a.id_usuario=b.id_usuario  and (b.id_dependencia = a.id_destino or b.id_dependencia =a.id_procedencia)  and a.id_seguimiento in (select min(id_seguimiento) from tramite where id_documento=$doc2)
