<?php
	...
	if ($id_funcionario!=1419){// OJO, que dice distinto pajaron!!!!!
	 $rs ="SELECT *,convert(varchar,d.fecha_documento,103) as fecha_dco, REPLACE(materia, CHAR(13) + CHAR(10), '') AS materia2,
			...
			WHERE (t.id_destino = '".$id_dependencia."' AND t.tipo_destinatario='I') AND (t.id_estado_tramite = 2 OR t.id_estado_tramite = 3) 
			AND ((YEAR(t.fecha_despacho) = ".$txtagno.") OR (YEAR(t.fecha_despacho) = ".($txtagno-1)."))
...
// se cambia para formato de sql server 2005 
		$rs="SELECT COUNT(tipo_dia) AS cantidad FROM calendario where fecha BETWEEN ".$array_fecha[$i]." AND convert(varchar(10),GETDATE(),112) AND (tipo_dia = 'H')";
		$rs0 =@mssql_query($rs);
