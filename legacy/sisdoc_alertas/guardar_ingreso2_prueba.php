...
// buscando tipo de documento en tabla 
   $tipo_d="select desc_tipo_documento from tipo_documento where  id_tipo_documento= ". $Cbo_Tipo_Docto;
   ...
   $reg_tipo = mssql_fetch_array($r_tipo_d);
		...
		$id_nomina =0;
        $TxtObservacion = "Se adjunta con ". $reg_tipo[desc_tipo_documento] .  "  N° " . $num_interno ; 
		...
		$TxtDias . "','" . '' . "','" . ' ' . "','" . $fechasistema . "','" . $fechasistema . "','" .
		ltrim($TxtObservacion) . "','" . $op . "'";
//	    echo $tramite_factura_query;
		$rs_tram_fact=mssql_query($tramite_fact_query,$cn); 
	    $reg_tram_factura=mssql_fetch_array($rs_tram_fact);
		...
		$compromiso=2;
	     $fechasistema = date("Y/m/d H:i"); 
		$rs_estado="exec mod_estado_detalle_factura '" . $idseguim. "','" . $fechasistema. "','" . $estado . "','" . $opcion . "','" .$op . "','" . $compromiso. "'";
		// echo $rs_estado;
		...
		// echo  $relacion_factura_documento;
///////
...
echo '<html><body onload="document.form1.submit();">';
	echo '<form name="form1" method="post" action="ingreso_documento_refrendacion.php">' . "\n";
	...
	echo '<input type="hidden" name="txtexped" value="' . $numexp . '">' . "\n";
		...
		echo '<input type="hidden" name="Cbo_Tipo_documento" value="' . $Cbo_Tipo_Documento . '">' . "\n";
	echo '</form></body></html>'  . "\n";
}	
