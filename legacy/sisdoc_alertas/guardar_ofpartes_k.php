...
//echo "procedencia  " . $Cbo_Procedencia  . "cbo_esc_proc" . $cbo_esc_proc  . "cbo_proc" . $cbo_proc;
$flujo = 0;
...
{ 
#----------- guardar la relacion de documentos si este existe al momento de ser reiteración de documento -------
if ($iddoc2<>'')
{
// busca el ultimo documento relacionado al origen para pegar el documento nuevo en la cola de los tramites 

$ps_busca_padre = "select * from llama_historia($iddoc2)";
$rs_p = mssql_query($ps_busca_padre,$cn); 
while($rss=mssql_fetch_array($rs_p))
 { $ultimo_doc= $rss["id_documento"]; }
 //
$relacion_query ="exec guardar_relacion '"  .  $Id_Documento.  "','" . $ultimo_doc . "'";
$rs_relacion = mssql_query($relacion_query,$cn); 
}


#---------Guardar fecha del timbre de recepcion por parte de oficina de partes  09/07/2007----------#
$dia = substr($Txt_fecha_timbre,0,2);
$mes = substr($Txt_fecha_timbre,3,2);
$año = substr($Txt_fecha_timbre,6,4);
$Fecha_timbre= $año .$mes. $dia ;
//echo  "fechatimbre" . $Fecha_timbre . "dia " . $dia  . "mes " . $mes . "aa" . $año . "<br>";
$fecha_recepcion_query="exec ingreso_fecha_timbre_recepcion '" . $Id_Documento . "','" . $Fecha_timbre .  "'";
$rs_recepcion = mssql_query($fecha_recepcion_query,$cn); 
//echo "query" . $fecha_recepcion_query;
...
$compromiso_tramite="exec ingreso_compromiso_tramite '" . $Id_Documento . "','" . $idcomp . "'";
$rs_comprom = mssql_query($compromiso_tramite,$cn); 
}
}
else
