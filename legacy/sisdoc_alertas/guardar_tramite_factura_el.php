...
{
   $grabado=0;
   $query_doc="select max(id_detalle) as doc from detalle_facturas where id_factura =" . $id_doc;
   ...
   if($reg_doc["doc"]==$id_seg)
   {
		...
		$rs_tram="exec busca_tramite_factura_el '" . $id_seg . "'";
	...
	$datos_query = "exec busca_datos_factura '" . $id_seg . "'" ; 
	...
	$documento_query = "exec ing_respaldo_factura '" . $reg_datos["id_factura"] . "','" . $reg_datos["id_tipo_fact"] . "','" .  $reg_datos["id_tema_fact"] . "','" .
	$reg_datos["id_usuario"] . "','"  . $reg_datos["num_factura"] . "','" .
	$reg_datos["fecha_factura"] . "','" .  $reg_datos["fecha_sistema"] . "','" .
	ltrim($reg_datos["descripcion"]) . "','" . $reg_datos["id_detalle"] . "','" . $reg_datos["id_procedencia"] . "','" .
...
//echo $documento_query;
	...
	$ret_doc=$reg_doc["ret"];
	   // si se respaldo bien va a eliminar los descriptores,tramites y documento 
		...
		if ($ret_doc==0)
		{
		$eliminacion_query = "exec elimina_documentos_tramites_facturas '" . $id_doc . "','" . $id_seg . "','" . $tipo_elim .  "'"; 
...
$sw_grabado = 2;
	...
	echo '<form name="form1" method="post" action="tra_enc_factura_el.php">' . "\n";
	...
	echo '<input type="hidden" name="Cbo_temas_fact" value="' . $tema_fact . '">' . "\n";
	echo '<input type="hidden" name="Num_factura" value="' . $Num_factura . '">' . "\n";
	echo '</form></body></html>'  . "\n";
