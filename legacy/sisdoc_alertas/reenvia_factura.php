...
/*echo 'cusuario' . $cusuario . 'idusuario ' . $idusuario .  '&materiaold=' . $materiaolda .  '&avanzada=' . $avanzada ."<br>";
echo '&iddocum=' . $iddocum . '&idseguim=' . idseguim .   '&idfuncionario=' . $idfuncionario . '&fechaini=' . $fechaini . '&fechafin=' . $fechafin . "<br>";
echo '&TxtMateria=' . $TxtMateria . '&avanza=' . $avanza. '&arreglo=' . $arreglo . '&mi_dependencia =' . $mi_dependencia . '&sw_cons =' . $sw_cons . "<br>"; 
echo '&Cbo_Tipo_Docto=' . $Cbo_Tipo_Docto . '&TxtInterno=' . $TxtInterno .'&TxtOficial=' . $TxtOficial .'&TxtExterno=' . $TxtExterno . "<br>";
echo '&desc=' . $desc .'&tipo_procedencia=' . $tipo_procedencia . ' &tipo_destino=' . $tipo_destino . ' &CboDestinatario=' . $CboDestinatario . ' &Cbo_Procedencia='. $Cbo_Procedencia . "<br>";
  */     
// busca accesos para poder derivar desde la opcion derivar  de las busquedas de tramites //
$proc_min = "select id_procedencia,tipo_procedencia  from detalle_facturas where  id_factura =" . $iddoc . " and id_detalle=" . $idseguim ;
//echo $proc_min; 
$reg_proc = mssql_query($proc_min);
$reg = mssql_fetch_array($reg_proc);
$proc_min = $reg[id_procedencia];
$tipo_min =$reg[tipo_procedencia];
//$rs_tramite = "select * from tramite where id_documento= " . $iddoc .  " and ".  "id_destino=" . $proc_min . " and tipo_destinatario=" . "'I'" ;

if ($tipo_min=='I')
{
	$rs_tramite = "select * from detalle_facturas where id_factura=" . $iddoc .  " and ".  "id_destino=" . $proc_min . " and tipo_destinatario=".  "'" . $tipo_min . "'" ;
	...
	echo '<form name="form1" method="post" action="doc_enc_facturas.php">';
...
//	echo '<input type="hidden" name=" $con_archivo"            value="' . $con_archivo . '">'."\n";
	...
	{
	echo '<html><body onload="javascript:document.form1.submit()">';
    	echo '<form name="form1" method="post" action="cambia_estado3_k3_factura.php">';
    	echo '<input type="hidden" name="idusuario"     	value="' . $idusuario . '">'."\n";
	echo '<input type="hidden" name="cusuario"      	value="' . $cusuario . '">'."\n";
