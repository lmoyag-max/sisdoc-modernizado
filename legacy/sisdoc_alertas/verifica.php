...
$y="";
 ...
 // busca por rut en tabla de usuario de sisdoc //
  ...
  if (isset($flag)){
		...
		echo '<form name="form1" method="post" action="verifica_antes.php">';
		echo '<input type="hidden" name="rut" value="' . $rut . '">';
		echo '<input type="hidden" name="aux_rut" value="' . $aux_rut . '">';
		echo '<input type="hidden" name="clave" value="' . $clave . '">';
		echo "</form></body></html>";
  ...
  // extrae tema de factura del usuario  // 08/07/2009
  $rs_tema= "select * from perfiles_facturas where id_usuario=". $id_usuario;
  $rg =mssql_query($rs_tema,$cn);
  $regperfil=mssql_fetch_array($rg);
  $id_tema = $regperfil[id_tema];
   
  $totperf=mssql_num_rows($rg);
   //echo "usuario" . $id_usuario . "tema " . $id_tema  . "total". $totperf;
 // buscando  factura en alertas 
 $dias =0;
 $rojos=0;
 $verdes=0;
 $amarillos=0;
...
if ($totperf <> 0)
{ 
$busca_fac="select a.* from facturas a , detalle_facturas t  where a.id_tema_fact =". $id_tema . " and (t.id_estado_tramite = 2 OR t.id_estado_tramite = 3)  and a.id_factura =t.id_factura  and  (YEAR(t.fecha_despacho) = " .$txtagno . ")";
//echo $busca_fac;
$rfact=mssql_query($busca_fac,$cn);
  
  while($reg_facturas = mssql_fetch_array($rfact)) 
		  {  
		  // buscando la cantidad de dias que lleva la fuactura en el sistema 
		        $fec_doc=strtotime($reg_facturas[fecha_recepcion]);
				...
				$diasx= $rd["total"];
				if   ($diasx >30)   { $rojos =$rojos +1; }
				else  if ($diasx==15 )  { $amarillos =$amarillos+1;}
				else  if ($diasx <31)   { $verdes =$verdes+1; } 
		   }		
  ...
  // fin cambio 
	if ($tot_dep == 0)
	{
		if ($tipo_menu <>'C')
		{
			...
			{  	
			 if ($totperf <> 0)// para qiue solo arroje el mensaje a  usuarios con perfil de las facturas (ejecutivos)
			{	echo '<script>';
			 //echo 'alert("Facturas verde : '. $verdes. '" )';
			 echo 'alert("Facturas verde : '. $verdes.  ' Amarillas : '.$amarillos . ' Rojos : '.$rojos .'" )';
			 echo '</script>';
			 }
			 echo '<html><body onload="document.form1.submit();">';
