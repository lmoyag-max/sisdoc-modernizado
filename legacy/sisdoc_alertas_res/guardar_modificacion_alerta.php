...
*/
	...
	echo '<form name="form1" method="post" action="modifica_tramite_alerta.php">';
...
//echo "fecha oficial" . $Txt_fecha_ofi . "<br>";	
if ($Txt_fecha_ofi > "01/04/2004")
   {
	$dia = substr($Txt_fecha_ofi,0,2);
	$mes = substr($Txt_fecha_ofi,3,2);
	$año = substr($Txt_fecha_ofi,6,4);
	$Fecha_ofi = date("Y/m/d H:i", mktime(0, 0,0, $mes, $dia, $año));
	}
else
{ $Fecha_ofi=NULL;}
...
#---- modifica fecha timbre recepcion ---#

$Fecha_timbre = substr($Txt_fecha_timbre,6,4). substr($Txt_fecha_timbre,3,2).substr($Txt_fecha_timbre,0,2);
//echo "fecha timbre1  " . $Txt_fecha_timbre  .  "fecha timbre2  " . $Fecha_timbre; 
$fecha_recepcion_query="exec ingreso_fecha_timbre_recepcion '" . $iddocumento . "','" . $Fecha_timbre .  "'";
...
if ($reg_desc[0]!=0){
	...
	echo '<html><body onload="document.form1.submit();">';
	echo '<form name="form1" method="post" action="modifica_cabecera2_alerta.php">';
	echo '<input type="hidden" name="idusuario" value="' . $xx . '">';
	echo '<input type="hidden" name="cusuario" value="' . $c_usuario . '">';
	echo '<input type="hidden" name="idfuncionario" value="' . $fun . '">';
	echo '<input type="hidden" name="flujook" value="' . $sw . '">';
